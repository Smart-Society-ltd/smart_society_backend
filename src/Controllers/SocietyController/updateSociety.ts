  ...
  try {
    ...
    if (!loggedInUser || loggedInUser.role != "admin") {
      return res.status(403).json({
        errorMsg: "Unauthorized: Only Admin can update Society Details",
      });
    }

    const society = await Society.findOne({
      society_code: req.body.society_code,
    });
    ...
    res
      .status(200)
      .json({ msg: "Society details updated successfully", data: society });
  } catch (error) {
